...
public interface DeletedMessageIdDao {
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM deleted_msg_for_me WHERE ownerUserId = :ownerUserId AND firebaseMessageId = :firebaseMessageId LIMIT 1)")
    ...
    @Query("DELETE FROM deleted_msg_for_me WHERE ownerUserId = :ownerUserId AND firebaseMessageId = :firebaseMessageId")
    ...
    @Query("DELETE FROM deleted_msg_for_me WHERE ownerUserId = :ownerUserId")
